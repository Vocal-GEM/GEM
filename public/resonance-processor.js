        ...
        class ResonanceProcessor extends AudioWorkletProcessor {
            ...
            analyze() {
                ...
                if (rms > this.threshold) {
                    ...
                    // In a real app, we'd use root solving on the LPC polynomial
                    let p1 = { freq: 0, amp: 0 }, p2 = { freq: 0, amp: 0 };
                    // ... (Peak picking logic omitted for brevity, using centroid for now) ...

                    // Calculate Vocal Weight (RMS / Amplitude)
                    const weight = Math.min(1, rms * 5); // Gain up

                    this.port.postMessage({ type: 'update', data: { pitch, resonance: this.smoothedCentroid, f1: 0, f2: 0, weight, spectrum: lpcSpec } });
                } else {
