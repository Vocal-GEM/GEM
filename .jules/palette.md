...
## 2024-05-25 - Floating Components Accessibility
**Learning:** Floating, draggable widgets (like camera previews) often contain tiny, icon-only control buttons (zoom, resize, close) that are completely inaccessible to screen readers if missing `aria-label` attributes, as their visual context isn't parsed.
**Action:** Always ensure floating overlay controls have descriptive `aria-label`s, even if they are only revealed on hover for sighted users.
