...
## 2026-02-18 - Tooltip Accessibility
**Learning:** Purely visual tooltip trigger icons often lack keyboard accessibility (focus/blur) and screen-reader context.
**Action:** Always wrap interactive tooltip icons in `<button>` elements with `type="button"`, appropriate `aria-label`, `aria-expanded` state, and handle both mouse and keyboard events (hover and focus/blur) for robust accessibility.
