...
## 2024-05-24 - Missing ARIA Labels on Icon-only Mobile Toggles
**Learning:** Found that the primary mobile navigation toggle (`Menu`/`X` icon button in `Sidebar.jsx`) was lacking an accessible name and state, making the core navigation mechanism opaque to screen reader users.
**Action:** Always add `aria-label` (dynamic based on state) and `aria-expanded` attributes to icon-only toggle buttons to ensure the primary UI navigation remains fully accessible.
