...
## 2025-02-18 - Accessibility of Custom Toggles
**Learning:** Custom UI toggles built with `div`s or `button`s often lack the `role="switch"` and `aria-checked` attributes, causing screen readers to misidentify them as generic buttons without communicating their current on/off state.
**Action:** When implementing custom toggle switches, always include `role="switch"`, dynamically set `aria-checked`, and provide a clear `aria-label` if the visible label isn't programmatically associated.
