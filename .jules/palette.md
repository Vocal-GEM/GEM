...
## 2026-03-18 - Client Dashboard Icons
**Learning:** Icon-only buttons without proper ARIA attributes make it difficult for screen reader users to navigate management dashboards.
**Action:** Always add `aria-label` to icon-only buttons, and use `aria-hidden="true"` on the SVG itself to prevent redundant announcements.
