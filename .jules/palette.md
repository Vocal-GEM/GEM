...
## 2025-02-18 - ARIA Labels for Icon-Only Action Buttons
**Learning:** Critical interaction buttons (like play, record, stop, and close) in exercise components are often purely icon-based without proper aria-labels, making them completely inaccessible to screen readers.
**Action:** When adding or modifying interactive exercises, verify that all icon-only control buttons have descriptive aria-labels that update dynamically if their state changes (e.g., Start vs Stop recording).
