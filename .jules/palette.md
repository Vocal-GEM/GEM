...
## 2024-05-24 - Accessibility for Mobile Hamburger Menus
**Learning:** Icon-only disclosure widgets like mobile hamburger menus often lack state communication. Screen reader users need to know both what the button does (via `aria-label`) and its current state (via `aria-expanded`).
**Action:** Always include both a dynamic `aria-label` (e.g., 'Open menu' or 'Close menu') and `aria-expanded={isOpen}` attribute on mobile navigation toggle buttons.
