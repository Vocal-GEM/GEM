...
## 2026-01-20 - Icon-Only Mobile Navigation Buttons
**Learning:** Icon-only navigation toggles (like a hamburger menu) without `aria-label`s are completely opaque to screen readers, preventing users from accessing core navigation.
**Action:** Always add descriptive `aria-label`s that reflect the *current state* of the toggle (e.g., "Open navigation menu" vs "Close navigation menu") and ensure they have visible focus indicators (`focus-visible:ring-2`).
