...
## 2024-05-23 - Info Tooltip Accessibility
**Learning:** Icon-only Info/Help tooltips are often implemented as unsemantic `div`s with hover events, missing keyboard focus, roles, ARIA descriptors, and explicit keyboard controls (`Enter`/`Escape`).
**Action:** When implementing tooltips, always add `role="button"`, `tabIndex={0}`, `aria-expanded`, `aria-describedby` linked to the tooltip content, and a comprehensive `onKeyDown` handler that supports toggling with `Enter`/`Space` and closing with `Escape`.
