...
## 2024-05-24 - Unlabelled Icon Buttons
**Learning:** Icon-only buttons are a common pattern in the codebase, but often lack `aria-label` attributes. This creates a significant barrier for screen reader users, who will only hear "button" without context.
**Action:** When working with buttons, always verify that icon-only instances include a descriptive `aria-label` to convey the button's purpose clearly.
