...
## 2025-06-07 - Toggle Button Accessibility
**Learning:** Icon-only toggle buttons often lack proper ARIA labels and focus indicators, making them difficult to use for keyboard-only or screen reader users.
**Action:** When working with toggle buttons (like mode switchers or debug panels), always ensure they have dynamic `aria-label`, `aria-expanded` (if they reveal content), and `focus-visible` utility classes for clear visual feedback.
