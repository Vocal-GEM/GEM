...
## 2026-04-15 - Skeleton Status Verbosity
**Learning:** Adding `role="status"` to base `Skeleton` components makes them accessible but can create excessive noise for screen reader users when rendering multiple skeletons simultaneously (like in `ListSkeleton`).
**Action:** When creating grouped loading skeletons, add `aria-hidden="true"` to individual skeleton elements and apply a single `role="status"` with a visually hidden label to the parent container.
