...
## 2024-05-25 - Accessibility of Voice Analytics Refresh Button
**Learning:** Purely visual icon-only buttons (like the `RefreshCw` button in `VoiceFingerprint.jsx`) lacking ARIA labels leave screen reader users unaware of their purpose, turning critical interactions like reloading voice analytics into "silent" controls.
**Action:** Always verify that icon-only buttons include `aria-label` providing clear context (e.g., "Refresh voice analytics") and appropriate `title` attributes for tooltips, along with explicit `focus-visible` states to enhance keyboard navigation.
