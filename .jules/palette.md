...
**Learning:** Icon-only buttons used for tooltips or info toggles often lack accessible labels, rendering them silent and confusing for screen reader users.
**Action:** Always add `aria-label` to purely visual or icon-only buttons to convey their purpose to assistive technologies.
