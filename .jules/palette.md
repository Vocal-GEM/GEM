...
**Learning:** Found multiple icon-only buttons across components (`IntonationTrainer`, `ClientDashboard`, `FloatingCamera`, `VoiceAudit`) missing `aria-label` attributes, which makes them inaccessible to screen readers.
**Action:** Always add explicit `aria-label` attributes to buttons that only contain icons and no text, describing the action they perform.
