...
## 2024-05-31 - Icon-Only Toggle Accessibility
**Learning:** Components that serve as floating, icon-only toggles (like `OrbLegend`) often lack contextual `aria-label`s, rendering them completely opaque to screen readers.
**Action:** Always provide a dynamic `aria-label` (e.g., "Open [Feature]" / "Close [Feature]") to toggle buttons that rely purely on visual icons to convey their state.
