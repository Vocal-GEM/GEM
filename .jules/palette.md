...
**Learning:** Foundational components using icons for actions (e.g. back buttons, close buttons, toggle views) often lack `aria-label` attributes, rendering them invisible or confusing to screen readers.
**Action:** When working with buttons that only contain an icon, consistently ensure an `aria-label` is present to convey the action clearly.
