...
## 2025-02-18 - Keyboard Accessible Tooltips
**Learning:** Tooltips that rely solely on `onMouseEnter` and `onMouseLeave` attached to `div` or icon elements are invisible to keyboard-only and screen reader users because they cannot receive focus.
**Action:** Always wrap tooltip trigger icons in a `<button>` element, add `onFocus` and `onBlur` event handlers to match mouse events, and include `aria-expanded` and an appropriate `aria-label` to communicate state and purpose to screen readers.
