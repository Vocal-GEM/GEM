...
## 2025-02-18 - Icon Button Accessibility
**Learning:** Icon-only buttons (like those using `lucide-react` icons) are completely invisible to screen readers unless they have explicit `aria-label` attributes.
**Action:** When auditing or building components, always ensure that any `<button>` element lacking text content has a descriptive `aria-label` or visually hidden text.
