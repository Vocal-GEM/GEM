...
## 2025-02-18 - Settings Toggle Accessibility
**Learning:** Toggle buttons styled as complex UI elements without native input types lose their semantic meaning, leaving screen reader users unaware of their state or function.
**Action:** Always ensure custom toggle buttons include `role="switch"` and dynamically update `aria-checked` to convey their true purpose and current state.
