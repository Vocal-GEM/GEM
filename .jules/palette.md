...
## 2026-07-29 - Mobile Menu Toggle Accessibility
**Learning:** The mobile menu toggle button (`<button>` wrapping the `Menu` icon) was missing an `aria-label` and `aria-expanded` attribute, leaving screen reader users without context on what the button does or its current state.
**Action:** Added an `aria-label` that dynamically updates based on the `isOpen` state ('Open menu' or 'Close menu'), included `aria-expanded={isOpen}`, and added focus-visible styles (`focus-visible:ring-2 focus-visible:ring-blue-500`) to improve keyboard navigation accessibility.
