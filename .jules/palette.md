...
## 2026-06-01 - Dynamic ARIA labels on Toggle Buttons
**Learning:** Toggle components like Play/Pause buttons require dynamic `aria-label` attributes that reflect the *action* that will occur when clicked (e.g., "Pause" when playing), rather than a static description.
**Action:** Always use conditional rendering (e.g., `isRunning ? "Pause" : "Play"`) for `aria-label`s on icon-only toggle buttons to ensure accurate screen reader announcements.
