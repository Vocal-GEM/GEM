...
**Learning:** Icon-only buttons (like Save/Cancel actions mapped to `<Save />` or `<Trash2 />` icons) frequently lack descriptive text, rendering them invisible or confusing to screen reader users.
**Action:** When auditing or creating UI components with icon-only controls, always verify and explicitly add a descriptive `aria-label` attribute to the wrapping `<button>` element.
