...
## 2025-02-18 - Tooltip Button Accessibility
**Learning:** Info icon buttons with custom hover tooltips are completely inaccessible to keyboard users and screen readers if they only use `onMouseEnter`/`onMouseLeave` and lack an `aria-label`.
**Action:** Always add `aria-label`, pair mouse events with `onFocus`/`onBlur` for keyboard support, add `aria-expanded`, and include `focus-visible` styles to ensure visual keyboard focus.
