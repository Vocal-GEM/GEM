...
## 2024-05-20 - Accessible Tooltips for Icon-Only Buttons
**Learning:** For icon-only buttons that trigger tooltips, using `aria-expanded` is semantically incorrect (it implies a collapsible element). The correct approach requires linking the button to the tooltip using `aria-describedby` pointing to the tooltip's `id`.
**Action:** When adding tooltips to interactive elements, ensure the trigger has an `aria-label`, an `aria-describedby` attribute, and `onFocus`/`onBlur` handlers. The tooltip container must have a matching `id` and `role="tooltip"`.
