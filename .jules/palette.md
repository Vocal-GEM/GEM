...
## 2024-03-24 - Accessible Mobile Menu Toggle
**Learning:** Icon-only disclosure widgets like mobile hamburger menus require both dynamic `aria-label` (e.g., 'Open menu'/'Close menu') and `aria-expanded` attributes to properly communicate their function and current state to screen reader users.
**Action:** Always include state-aware accessibility attributes for icon-only toggle buttons.
