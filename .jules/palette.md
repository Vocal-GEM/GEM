...
## 2026-06-29 - Replaced Alert in Verification File
**Learning:** Even in internal verification or test sandbox files, using `window.alert` creates a jarring UX. It's better to implement a temporary Toast state to verify the component behaves as expected without blocking the thread.
**Action:** Always replace `window.alert` with the `Toast` component or `console.log` across all files to maintain a consistent UX standard.
