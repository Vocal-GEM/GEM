...
## 2025-05-24 - Accessible Alerts
**Learning:** Using native `window.alert()` creates a blocking, inaccessible experience that interrupts the user flow and isn't read correctly by all screen readers.
**Action:** Always use the custom `Toast` component instead of `alert()`, ensuring it has the correct `role="alert"` for error messages, which provides non-blocking, accessible feedback.
