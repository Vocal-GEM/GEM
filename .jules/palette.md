...
## 2024-05-24 - Add ARIA Labels to Icon-Only Buttons in IntonationTrainer
**Learning:** Found several icon-only buttons (History, Prev/Next Pattern) in `IntonationTrainer` missing `aria-label` attributes, which prevents screen readers from announcing their function. Also added `aria-expanded` to the history toggle button to indicate state.
**Action:** Always ensure icon-only buttons have descriptive `aria-label` attributes, and stateful toggle buttons use `aria-expanded` to communicate their current state.
