...
## $(date +%Y-%m-%d) - Accessible Icon-Only Legend Buttons
**Learning:** Icon-only toggle buttons for informational overlays (like legends or visual keys) often lack context for screen readers, making it unclear what the button does or whether the overlay is currently open.
**Action:** Always add dynamic `aria-label` (e.g., "Open visual key" / "Close visual key"), `aria-expanded`, and explicit `focus-visible` styles to floating icon-only toggle buttons to ensure they are discoverable and their state is clear to keyboard and screen reader users.
