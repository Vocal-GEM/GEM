...
## 2026-05-25 - Icon-only Buttons Accessibility
**Learning:** Foundational components using icons without labels (like `X` for close, `ZoomIn` for zoom) are common in this app and create silent/confusing elements for screen reader users.
**Action:** Always add `aria-label` to icon-only buttons to ensure they have an accessible name.
