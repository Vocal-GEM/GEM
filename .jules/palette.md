...
## 2026-06-25 - Accessibility in Custom Tooltips
**Learning:** Custom tooltips that rely solely on `onMouseEnter` and `onMouseLeave` are inaccessible to keyboard and screen reader users.
**Action:** Always pair mouse hover events with `onFocus` and `onBlur`, and add `aria-expanded` and descriptive `aria-label` attributes to the trigger elements.
