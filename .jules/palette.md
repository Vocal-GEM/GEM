...
## 2024-05-25 - Icon-only Buttons and Screen Readers
**Learning:** Foundational UI elements like a close or refresh button are frequently represented as just an SVG icon, rendering them invisible or confusing to assistive technology.
**Action:** When adding or auditing icon-only interactive elements, ensure an `aria-label` is applied to the `<button>` and the decorative icon receives `aria-hidden="true"`. Additionally, explicitly add `focus-visible` styles like `focus-visible:ring-2` to support keyboard navigation.
