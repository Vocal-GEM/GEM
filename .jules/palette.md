...
## 2025-02-18 - Missing Interactive States and Attributes in Navigation Menus
**Learning:** High-level layout components like `Sidebar` often lack dynamic accessibility attributes for toggling states (`aria-expanded`) and active item indication (`aria-current`). This causes screen readers to announce navigation menus simply as lists of buttons, lacking the context of what is currently open or selected.
**Action:** When auditing or fixing navigation sidebars and custom dropdowns, always ensure toggle buttons accurately reflect their state via `aria-expanded` and that the currently active page is explicitly marked with `aria-current="page"`.
