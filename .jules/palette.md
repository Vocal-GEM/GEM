...
**Learning:** Purely visual, icon-only buttons (like info or debug panel toggles) are inaccessible to screen readers without an `aria-label` attribute, creating silent interaction points for users relying on assistive technology.
**Action:** Always add an explicit, descriptive `aria-label` to buttons containing only icons to ensure proper keyboard and screen reader accessibility.
