...
## 2025-02-18 - Icon-only button accessibility in interactive exercises
**Learning:** Icon-only buttons (like back, close, or record buttons) in interactive exercises often lack `aria-label` and `focus-visible` styles, making them inaccessible to screen readers and difficult to navigate via keyboard.
**Action:** Always add `aria-label` and `focus-visible:ring-2 focus-visible:outline-none` classes to all icon-only interactive elements.
