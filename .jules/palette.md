...
## 2024-05-24 - Accessible Custom Tooltips
**Learning:** Custom tooltips triggered only by `onMouseEnter` and `onMouseLeave` are invisible to keyboard users. Furthermore, icon-only info buttons without `aria-label` or `aria-expanded` attributes are poorly understood by screen readers.
**Action:** Always pair `onMouseEnter`/`onMouseLeave` with `onFocus`/`onBlur` for custom tooltips on interactive elements. Add descriptive `aria-label` and `aria-expanded={state}` attributes, and apply `focus-visible` styles to ensure keyboard accessibility.
