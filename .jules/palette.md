...
## 2024-09-07 - Sidebar Mobile Menu Accessibility
**Learning:** The mobile menu toggle button (`Sidebar.jsx`) was lacking accessibility attributes, preventing screen readers from understanding its function and state.
**Action:** Added `aria-label` that dynamically updates based on the `isOpen` state ('Open menu' / 'Close menu') and an `aria-expanded` attribute. This is a common pattern for icon-only disclosure widgets.
