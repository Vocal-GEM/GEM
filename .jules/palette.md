...
## 2024-05-18 - ARIA Labels on Visualization Controls
**Learning:** Icon-only buttons in complex WebGL/Canvas visualization components (like `DynamicOrb`, `BrightnessMeter`, `HighResSpectrogram`) are frequently overlooked for accessibility because they use generic `lucide-react` icons and only have `title` attributes.
**Action:** Always add `aria-label` to icon-only buttons, and pair them with `aria-pressed` (for toggles) or `aria-expanded` (for tooltips) to provide full context to screen readers interacting with these highly visual tools.
