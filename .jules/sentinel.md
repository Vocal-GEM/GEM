...
## 2026-07-06 - Missing Variables and Unescaped Entities in Analytics and Audio Files
**Vulnerability:** Although not directly exploitable as a security vulnerability, unused imports/variables and unescaped entities (e.g., quotes in JSX) clutter the codebase, trigger CI pipeline failures, and indicate poor code hygiene, which can obfuscate actual security vulnerabilities or logic errors.
**Learning:** CI failures due to strict linting rules (like  and ) block the deployment of important security patches. In WebAudio  environments, the global context is restricted, and  must be accessed explicitly via  to prevent runtime errors. Similarly, JSX requires text quotes to be properly escaped with HTML entities (, ).
**Prevention:** Consistently run targeted linters locally (e.g., ) before committing code to ensure strict adherence to project standards, use proper HTML entities for text quotes in React components, and explicitly reference the  object for globals within isolated contexts like AudioWorklets.
## 2026-07-06 - Strict Linting, Isolated Contexts, and Escaped Entities
**Vulnerability:** CI failures due to strict linting rules (`no-undef`, `react/no-unescaped-entities`, `no-unused-vars`) block the deployment of important security patches.
**Learning:** In WebAudio `AudioWorkletProcessor` environments, the global context is restricted, and `currentTime` must be accessed explicitly via `globalThis.currentTime` to prevent runtime errors. Similarly, JSX requires text quotes to be properly escaped with HTML entities (`&quot;`, `&apos;`).
**Prevention:** Consistently run targeted linters locally (e.g., `npx eslint@8 <files>`) before committing code to ensure strict adherence to project standards, use proper HTML entities for text quotes in React components, and explicitly reference the `globalThis` object for globals within isolated contexts like AudioWorklets.
## 2026-07-06 - Strict Linting, Isolated Contexts, and Escaped Entities
**Vulnerability:** CI failures due to strict linting rules (`no-undef`, `react/no-unescaped-entities`, `no-unused-vars`) block the deployment of important security patches.
**Learning:** In WebAudio `AudioWorkletProcessor` environments, the global context is restricted, and `currentTime` must be accessed explicitly via `globalThis.currentTime` to prevent runtime errors. Similarly, JSX requires text quotes to be properly escaped with HTML entities (`&quot;`, `&apos;`).
**Prevention:** Consistently run targeted linters locally (e.g., `npx eslint@8 <files>`) before committing code to ensure strict adherence to project standards, use proper HTML entities for text quotes in React components, and explicitly reference the `globalThis` object for globals within isolated contexts like AudioWorklets.
