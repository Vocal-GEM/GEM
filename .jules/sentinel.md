...
## 2024-03-14 - Fix Stored XSS in flag_content\n**Vulnerability:** User input for `reason` and `content_type` fields were being directly inserted into the database in the `flag_content` endpoint without sanitization, leading to a Stored XSS vulnerability.\n**Learning:** When creating database models from user-supplied JSON bodies, every string field must be explicitly sanitized, even if it is meant to be a simple enum or description field. Missing sanitization allows malicious scripts to be stored and executed when viewed by admins or other users.\n**Prevention:** Apply `sanitize_html(raw_value) if raw_value else None` to all optional string inputs in API routes before passing them to SQLAlchemy models.
