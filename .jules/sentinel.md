...
## 2026-02-14 - Marketplace Validation Gaps
**Vulnerability:** The `create_pack` endpoint in `marketplace.py` lacked validation for categorical fields (`category`, `target_audience`, `voice_goal`) and numerical constraints (`price_cents`). This allowed negative prices and potential XSS via invalid category values.
**Learning:** Relying on frontend UI to restrict values is insufficient. Backend must always enforce business logic constraints (allowlists for enums, range checks for numbers) to prevent data corruption and security bypasses.
**Prevention:** Implemented strict allowlist validation for all categorical fields and non-negative checks for price. Added regression tests to `test_marketplace_security.py`.
