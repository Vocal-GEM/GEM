...
## 2026-10-25 - Broken Logic Flow in Anonymization
**Vulnerability:** The `share_voice` endpoint in `backend/app/routes/community.py` contained syntax errors (nested `finally` blocks) and logic errors where anonymization was attempted before the file was saved. This would lead to server errors (DoS) or potential failure to delete the original raw audio file (Privacy Violation).
**Learning:** Complex error handling with `try...finally` can be error-prone if copied/pasted without verification. Syntax errors in infrequently used code paths (like specific routes) might go unnoticed if not covered by basic import/compilation tests.
**Prevention:** Always verify file integrity (e.g., `python -m py_compile`) and write tests that cover the "happy path" of critical functions to ensure basic logic flow (save -> process -> cleanup) is valid.
