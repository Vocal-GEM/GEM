...
## 2026-08-15 - Timing Attack in User Enumeration\n**Vulnerability:** The `/login` endpoint in `backend/app/routes/auth.py` skipped the `check_password_hash` function when a user was not found, resulting in a significantly faster response time compared to when a user existed. This allowed attackers to enumerate valid usernames by measuring response times.\n**Learning:** Timing differences in authentication endpoints are a common vector for user enumeration. When mitigating by using a dummy hash comparison, the dummy hash string should be dynamically generated at module level during startup (`DUMMY_PASSWORD_HASH = generate_password_hash('dummy')`) rather than hardcoded, to match the real computational delay accurately and prevent potential 500 errors on older Werkzeug versions.\n**Prevention:** Always ensure that authentication paths take a similar amount of time regardless of whether the user exists or not. Implement dummy password hash checks in the 'user not found' execution branch.
