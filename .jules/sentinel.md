...
## 2024-05-23 - Information Leakage via Raw Exception Strings in Multiple Endpoints\n**Vulnerability:** API routes (including `tts.py`, `voice_quality.py`, `community.py`, etc.) were directly returning raw exception strings (`str(e)`) inside `jsonify({'error': ...})` blocks. This exposes internal details like API response structures, error types, and potentially sensitive stack traces to external clients.\n**Learning:** Relying on basic `str(e)` output during exception handling in Flask endpoints acts as an unintentional conduit for information disclosure. Endpoints should default to generic failure messages for the client, while reserving detailed exception context strictly for internal application logs.\n**Prevention:** Always implement generic client-facing error messages in API responses (e.g., 'Failed to connect to API'). Use `current_app.logger.error(f'...: {str(e)}')` to securely log the full exception context server-side, preventing information leakage.
