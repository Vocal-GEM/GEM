...
## 2025-02-14 - [Information Leakage in Error Handlers]
**Vulnerability:** API error handlers in voice_quality.py were catching generic exceptions, cleaning up files, and returning the raw stringified exception `str(e)` directly to clients (e.g. `return jsonify({'error': str(e)}), 500`). Additionally, they used `print` instead of the Flask application logger. Some error handler blocks also had duplicated cleanup code that became unreachable due to an early `return` or redundant implementations.
**Learning:** Returning `str(e)` directly exposes backend internal state, potential file paths, and library specific errors to users which can aid attackers. The unreachable code blocks were created because cleanup logic was redundantly defined both before and after the `return` statement. Also `except:` without an indented block causes a `SyntaxError`, meaning any automated find-and-replace scripts must be highly precise to avoid disrupting python indentation or leaving empty blocks.
**Prevention:** Always log the detailed error internally using `current_app.logger.error(e)`, and return a static, generic error message (e.g. 'An internal error occurred during...') to the client. Ensure cleanup code correctly falls back or is managed inside a `finally` block or right before the return to guarantee execution.
