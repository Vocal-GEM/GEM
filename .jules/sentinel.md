...
## 2024-10-27 - Information Leakage in Global Error Responses and User Enumeration Vulnerability
**Vulnerability:** Found multiple instances where API error handlers directly exposed internal exceptions via `str(e)` in JSON responses (e.g., in `tts.py`), leaking internal state or infrastructure details. Also identified a user enumeration vulnerability in `auth.py` where invalid usernames returned faster than valid usernames with invalid passwords because the password hash check was entirely skipped.
**Learning:** Hardcoded responses and skipped cryptographic comparisons during early failure conditions can lead to subtle but reliable side-channel attacks (timing attacks). Using `str(e)` directly inside `jsonify()` guarantees sensitive info will eventually leak to the client on unexpected errors. The `werkzeug.security.check_password_hash` raises a `TypeError` if passed `None`, so `request.json` payloads must be defensively parsed (`data.get('password', '')`).
**Prevention:** Implement defense-in-depth by standardizing generic error messages across all endpoints and moving `str(e)` exclusively to server-side logs (`current_app.logger`). Mitigate timing attacks by pre-computing a module-level `DUMMY_PASSWORD_HASH` and performing a dummy `check_password_hash(DUMMY_PASSWORD_HASH, password)` when the requested user is not found in the database.
