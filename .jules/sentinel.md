...
## 2024-05-18 - Exception Handling Information Leakage & Resource Exhaustion Mitigation
**Vulnerability:** In `backend/app/routes/voice_quality.py`, exception handlers were exposing raw Python stack traces and internal error strings directly to the client via `return jsonify({'error': str(e)}), 500`. Furthermore, cleanup logic for temporary audio files in these exception handlers was either bypassed by the premature return, raising `UnboundLocalError`, or missing entirely in some blocks, leading to orphaned files accumulating and potentially causing disk space exhaustion (DoS).
**Learning:** Returning `str(e)` in Flask exception handlers is a common anti-pattern that exposes internal application state and paths to attackers. In endpoints processing uploaded files, custom cleanup logic outside a standard `finally` block or context manager is brittle, and unhandled exceptions can easily skip cleanup resulting in DoS vulnerabilities over time. Using `if var_name and os.path.exists(var_name):` without first checking `if 'var_name' in locals():` can also crash the error handler itself.
**Prevention:** Always log the raw exception internally (`logger.error(...)`) and return a generic, sanitized error message to the client. Use robust `finally` blocks for resource cleanup, and defensively check `if 'var_name' in locals() and var_name and os.path.exists(var_name):` before attempting to remove temporary files if they were initialized inside a `try` block that might have failed early.
## 2024-05-18 - Prevent Path Traversal / SSRF via Unsanitized Input in URL Paths
**Vulnerability:** In `backend/app/routes/tts.py`, user-provided input `voiceId` was directly concatenated into an external API URL path `f'https://api.elevenlabs.io/v1/text-to-speech/{voice_id}'` without any sanitization. This allows an attacker to inject path traversal characters (e.g., `../`) or arbitrary query strings, potentially exploiting SSRF or manipulating the external API request.
**Learning:** Never trust user input injected into URLs or external API endpoints. Even if the expected input is a simple alphanumeric ID, malicious actors can craft payloads that traverse paths or append unwanted query parameters.
**Prevention:** Always URL-encode user input when constructing URL paths. Using `urllib.parse.quote(str(user_input), safe="")` ensures that all special characters are safely encoded, mitigating injection risks.
