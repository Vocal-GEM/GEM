...
## 2026-01-24 - Layout Thrashing in PitchOrb.jsx Animation Loop
**Learning:** `getBoundingClientRect()` inside the `requestAnimationFrame` loop caused synchronous layout recalculations and repaints, drastically dropping framerate. In Canvas API, if you set `canvas.width` and `canvas.height`, it also implicitly clears the canvas. Refactoring dimensions outside the loop means we must manually manage state or ensure `ctx.clearRect()` is still called.
**Action:** Replaced `getBoundingClientRect` with a `ResizeObserver` setup that tracks component resizing asynchronously, caching dimensions into a `ref` used inside the loop, while still retaining `ctx.clearRect()`.
