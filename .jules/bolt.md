...
**Learning:** `getBoundingClientRect()` causes a synchronous reflow (layout thrashing) when called inside a high-frequency animation loop in `PitchOrb.jsx`. Additionally, setting `canvas.width` and `canvas.height` repeatedly clears canvas state.
**Action:** Used `ResizeObserver` to track element dimensions asynchronously. Updated canvas size only when dimensions change, wrapping draws with `save()`/`restore()`.
