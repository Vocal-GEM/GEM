...
**Learning:** `getBoundingClientRect()` causes a synchronous reflow (layout thrashing) when called inside a high-frequency animation loop (e.g., `renderCoordinator.subscribe`). In `PitchOrb.jsx`, this was being called in the rendering loop, which forces a full repaint and burns CPU cycles for no benefit, identical to what was found previously in `PitchVisualizer.jsx`.
**Action:** Use `ResizeObserver` to track element dimensions asynchronously. Update the canvas size only when the physical dimensions actually change, and store the dimensions in state/variables for use in the animation loop.
