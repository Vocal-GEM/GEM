...
## 2026-01-24 - Throttling React State in Animation Loops
**Learning:** Using `requestAnimationFrame` to poll a socket stream and update React state (`setState`) on every frame forces a 60FPS re-render of the component and its children, creating a severe performance bottleneck.
**Action:** When bridging high-frequency streams (like audio/socket data) to React state, implement a deadband (threshold checking) and rate-limit the actual state updates using `performance.now()` throttling. This maintains visual responsiveness while bypassing 80%+ of unnecessary render cycles.
