...
## 2026-05-21 - Resolving ESLint and Vitest specific CI failures
**Learning:** During linting, if a React component is mocked returning a plain arrow function (e.g. `vi.mock('lucide-react', ...)`), ESLint's `react/display-name` rule will fail the CI check. Also, `require` is not available in Vitest's ESM environment and triggers `no-undef`, and `global` needs to be mapped to `globalThis`. Unresolved merge conflicts leave duplicate declarations and broken syntaxes which can cause `SyntaxError` blocking the build.
**Action:** When mocking components, assign a `displayName`. Use dynamic `await import('xyz')` instead of `require('xyz')` inside `vi.mock` for ESM compatibility. Always replace `global.` with `globalThis.` in tests. Use robust git diff replacing to carefully remove exact duplicate lines left over by merge conflicts.
