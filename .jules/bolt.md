...
## 2026-01-25 - Redundant O(N) Array Iteration & Over-Optimization
**Learning:** Replacing idiomatic JavaScript array methods (`reduce`, `map`) with standard `for` loops on very small, capped arrays (e.g., `< 10` items) severely sacrifices code readability for zero measurable performance gains. However, passing pre-calculated aggregation variables (like mean) down into methods rather than repeating loops (e.g., passing `yMean` to `calculateRSquared`) prevents hidden redundant $O(N)$ operations.
**Action:** Do not micro-optimize array loops unless operating on large datasets. Always reuse precalculated sums or means rather than performing redundant reduce loops inside nested or sequential method calls.
**Action:** Never mutate public API return signatures (like adding a dummy return property) simply to bypass lint warnings. Use standard `// eslint-disable-next-line` suppression rules.