...
## 2025-05-23 - Layout Thrashing in Animation Loops 2
**Learning:** `getBoundingClientRect()` causes a synchronous reflow (layout thrashing) when called inside a high-frequency animation loop (e.g., `renderCoordinator.subscribe` callback). In `PitchOrb.jsx`, this was being called ~60 times per second along with canvas resizing, which forces a full repaint and state reset.
**Action:** Use `ResizeObserver` to track element dimensions asynchronously. Update the canvas size only when the physical dimensions actually change, and store the dimensions in a `ref` for use in the animation loop. Prevent canvas state reset by avoiding setting width/height unless they differ.
