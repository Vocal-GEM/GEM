...
## 2026-01-25 - Layout Thrashing in PitchOrb Animation Loop
**Learning:** `getBoundingClientRect()` causes a synchronous reflow (layout thrashing) when called inside a high-frequency animation loop. In `PitchOrb.jsx`, this was being called 60 times per second along with canvas resizing, which forces a full repaint.
**Action:** Used `ResizeObserver` to track element dimensions asynchronously. Updated the canvas size only when the physical dimensions actually change, and stored the dimensions in local variables for use in the animation loop.
