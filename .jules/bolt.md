...
**Learning:** `getBoundingClientRect()` causes synchronous reflow (layout thrashing) when called inside high-frequency animation loops. Additionally, continuously reassigning `canvas.width` and `canvas.height` forces a full canvas state reset and repaint on every frame.
**Action:** Use `ResizeObserver` to track element dimensions asynchronously. Store physical dimensions in a `dimensionsRef` to use inside the animation loop, and explicitly use `ctx.save()` and `ctx.restore()` since the state is no longer cleared via width reassignment.
