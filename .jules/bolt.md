...
## 2026-04-29 - Spectrogram3D/Vitest environment global variable fix\n**Learning:** Vitest uses ESM modules where `global` doesn't automatically map to `globalThis`. Referencing `global` inside tests (like setting global mocks for requestAnimationFrame) throws an unhandled reference error.\n**Action:** Use `globalThis` instead of `global` when declaring globals in Vitest test files to ensure compatibility across ESM module resolution.
