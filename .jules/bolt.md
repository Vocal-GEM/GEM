...
## 2026-01-24 - Eliminated Layout Thrashing in PitchOrb Render Loop
**Learning:** `PitchOrb.jsx` called `canvas.getBoundingClientRect()` and continually set `canvas.width` and `canvas.height` inside its high-frequency rendering loop. Modifying the canvas width implicitly clears its state and forces an expensive internal resize context operation, in addition to the forced reflow layout thrashing generated by `getBoundingClientRect()`.
**Action:** Relocated size calculations to an asynchronous `ResizeObserver`, caching dimensions within a `useRef` (`dimensionsRef`) exclusively for read access during the render loop. This drastically reduces CPU footprint.
