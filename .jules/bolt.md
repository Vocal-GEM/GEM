...
## 2025-05-23 - Canvas Resize vs Canvas Clear in Animation Loops
**Learning:** When removing continuous `canvas.width = rect.width` assignments from an animation loop (to fix layout thrashing), the canvas implicitly loses its automatic frame clearing. This causes rendering to smear as previous frames are no longer erased. Furthermore, implicit clearing also resets context state like `ctx.scale`.
**Action:** When fixing layout thrashing by moving canvas dimension assignments out of the render loop, you must explicitly manage frame clearing and state using `ctx.save()`, `ctx.resetTransform()`, `ctx.clearRect(0, 0, width, height)`, and `ctx.restore()` to ensure accurate and clean rendering.
