...
## 2025-10-24 - Visual Integrity Bug with Canvas Clearing
**Learning:** When removing `canvas.width` reassignments inside a high-frequency animation loop to prevent layout thrashing and context resets, it's crucial to realize that assigning a value to `canvas.width` implicitly completely clears the canvas. If removed, the old pixels remain, causing a visual smear/trail over time unless an explicit `ctx.clearRect` exists.
**Action:** When optimizing away `canvas.width = ...` calls in render loops, verify that an explicit `ctx.clearRect(0, 0, width, height)` is present to maintain component visual integrity.