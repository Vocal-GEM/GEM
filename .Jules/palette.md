...
**Learning:** Icon-only buttons (like 'Add' or 'Remove') are a common pattern that is completely invisible to screen readers without explicit labels.
**Action:** Audit all components using icons for actions and ensure every icon-only button has an `aria-label` describing its specific function (e.g., 'Remove phrase: [text]' instead of just 'Remove').
