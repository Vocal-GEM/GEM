...
const VoiceQualityMeter = ({ dataRef, userMode, showAnalysis = true }) => {
    ...
    // ⚡ Bolt Optimization: Track state in refs to avoid layout thrashing
    const currentLeftRef = useRef(0);
    const lastZoneRef = useRef(null);
    const lastValueRef = useRef(null);

    // Reset lastZone when colorBlindMode changes so we force a class update
    useEffect(() => {
    ...
    const loop = useCallback(() => {
        if (indicatorRef.current && valueRef.current && dataRef.current) {
            ...
            // Optimization: Skip DOM reads (layout thrashing)
            ...
            // Update DOM style (write)
            indicatorRef.current.style.left = `${nextLeft}%`;

            // Optimization: Only update class if zone changes
            let newZone = 'neutral';
            if (nextLeft > 70) newZone = 'heavy';
            else if (nextLeft < 30) newZone = 'light';
            ...
            // Optimization: Only update text if value changes
