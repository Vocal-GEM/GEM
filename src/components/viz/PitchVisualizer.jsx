...
const PitchVisualizer = React.memo(({ dataRef, targetRange, userMode, exercise, onScore, settings }) => {
    ...
    useEffect(() => {
        ...
        const loop = () => {
            ...
            // Draw Norms Zone (if enabled and available)
            // We assume 'settings.showNorms' exists or default to true for now to demonstrate
            const showNorms = settings?.showNorms !== false;

            if (showNorms) {
                // Determine gender ID from profile or settings
                // Assuming profile has a 'gender' or 'identity' field, or we infer from ID
                // For now, let's try to map the active profile ID to our norms
                let genderId = null;
                const pid = voiceProfiles.find(p => p.isActive)?.id; // This logic might need adjustment based on how activeProfile is stored

                // Fallback: Check if targetRange matches a known norm to infer label?
                // Or better, just show the norm that overlaps with targetRange?
                // Let's iterate and show the one that matches the user's goal best.

                // SIMPLIFICATION: Just show the norm for the current "targetRange" if it aligns with a gender
                // Actually, let's just show all 3 as faint background bands? No, too cluttered.
                // Let's show the one matching the user's "gender goal".
                // We don't have explicit gender goal in context yet.
                // Let's infer from targetRange center.
            ...
            // Draw Target Range (User's Personal Goal)
