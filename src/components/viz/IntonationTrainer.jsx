        ...
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-4">
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                    <button aria-label={showHistory ? "Hide history" : "Show history"} title={showHistory ? "Hide history" : "Show history"} onClick={() => setShowHistory(!showHistory)} className={`p-1.5 rounded hover:bg-slate-800 focus-visible:ring-2 focus-visible:outline-none ${showHistory ? 'text-blue-400' : 'text-slate-400'}`}>
                        <History size={16} />
                    </button>
                    <div className="flex items-center bg-slate-800 rounded px-1">
                        <button aria-label="Previous pattern" title="Previous pattern" onClick={prevPattern} className="p-1 hover:bg-slate-700 rounded focus-visible:ring-2 focus-visible:outline-none"><ChevronLeft className="w-4 h-4 text-slate-400" /></button>
                        <span className="text-xs text-slate-300 font-mono w-4 text-center">{selectedPattern + 1}</span>
                        <button aria-label="Next pattern" title="Next pattern" onClick={nextPattern} className="p-1 hover:bg-slate-700 rounded focus-visible:ring-2 focus-visible:outline-none"><ChevronRight className="w-4 h-4 text-slate-400" /></button>
                    </div>
                </div>
            </div>
