...
const HighResSpectrogram = memo(function HighResSpectrogram({ dataRef }) {
    ...
    const binIndexMapRef = useRef(null); // Optimization: Pre-calculated y -> bin index map
    const lastMaxBinRef = useRef(-1);
    ...
    const draw = useCallback(() => {
        ...
        // Removed 'willReadFrequently: true' to encourage GPU acceleration
        ...
        // Optimization: Pre-calculate bin indices if dimensions or FFT size changed
        // This removes 'height' floating point divisions per frame
        if (!binIndexMapRef.current || binIndexMapRef.current.length !== height || lastMaxBinRef.current !== maxBin) {
            binIndexMapRef.current = new Int32Array(height);
            lastMaxBinRef.current = maxBin;

            for (let y = 0; y < height; y++) {
                const freqRatio = (height - 1 - y) / height;
                binIndexMapRef.current[y] = Math.floor(freqRatio * maxBin);
            }
        }
