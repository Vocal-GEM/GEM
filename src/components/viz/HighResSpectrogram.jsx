...
const HighResSpectrogram = memo(function HighResSpectrogram({ dataRef }) {
    ...
    const binMapRef = useRef(null); // Cache for y-coordinate to bin index
    ...
    const draw = useCallback(() => {
        ...
        // 2. Draw new column
        // Optimized: Reuse pre-allocated TypedArray and pre-calculated bin map
        const maxBin = Math.floor(spectrum.length / 3);

        // Ensure bin map is ready and matches current height and maxBin
        if (!binMapRef.current || binMapRef.current.length !== height || binMapRef.current.maxBin !== maxBin) {
            binMapRef.current = new Int32Array(height);
            binMapRef.current.maxBin = maxBin; // Store maxBin to detect changes
