...
const MAX_FREQ = 8000;
const LOG_SCALE = 60 / Math.LN10; // Precompute constant for log10 conversion
...
const HighResSpectrogram = memo(function HighResSpectrogram({ dataRef }) {
    ...
    const lastDimensionsRef = useRef({ height: 0, spectrumLen: 0 });
    ...
    const draw = useCallback(() => {
        ...
        if (!imgDataRef.current || imgDataRef.current.height !== height) {
            try {
                ...
                // Reset binMap when height changes
        ...
        // Update binMap if needed
        if (!binMapRef.current ||
            lastDimensionsRef.current.height !== height ||
            lastDimensionsRef.current.spectrumLen !== spectrumLen) {

            binMapRef.current = new Uint16Array(height);
            const maxBin = Math.floor(spectrumLen / 3); // 8kHz cutoff
            ...
            lastDimensionsRef.current = { height, spectrumLen };
        }
        ...
        for (let y = 0; y < height; y++) {
            ...
            // Optimization: Math.log instead of Math.log10
            ...
            // Fill pixels - unrolled for scrollSpeed = 2
    ...
    useEffect(() => {
        ...
        const updateSize = () => {
            ...
            if (canvas.width !== newWidth || canvas.height !== newHeight) {
                ...
                // binMapRef.current will be reset in draw()
            }
        };
