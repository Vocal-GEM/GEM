...
const HighResSpectrogram = memo(function HighResSpectrogram({ dataRef }) {
    ...
    const componentId = useMemo(() => `spectrogram-highres-${uniqueId}`, [uniqueId]);

    // Reusable buffers to avoid garbage collection churn
    const imgDataRef = useRef(null);
    const data32Ref = useRef(null);
    // Optimization: Lookup table for frequency bin mapping to avoid per-pixel floating point math
    ...
    const draw = useCallback(() => {
        ...
        // Optimization: Recompute lookup table only if dimensions or maxBin changed
        if (!binIndexLookupRef.current || binIndexLookupRef.current.length !== height || lastMaxBinRef.current !== maxBin) {
            binIndexLookupRef.current = new Int32Array(height);
            for (let y = 0; y < height; y++) {
                const freqRatio = (height - 1 - y) / height;
                binIndexLookupRef.current[y] = Math.floor(freqRatio * maxBin);
            }
            lastMaxBinRef.current = maxBin;
        }

        const binIndexLookup = binIndexLookupRef.current;

        for (let y = 0; y < height; y++) {
            // Optimized: Use lookup table instead of per-pixel calculation
            const binIndex = binIndexLookup[y];
