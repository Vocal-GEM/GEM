...
const HighResSpectrogram = memo(function HighResSpectrogram({ dataRef }) {
    ...
    useEffect(() => {
        ...
        const updateSize = (rectWidth) => {
            const dpr = window.devicePixelRatio || 1;

            // Only update if dimensions actually changed
            const newWidth = Math.round(rectWidth * dpr);
        ...
        const resizeObserver = new ResizeObserver((entries) => {
            for (let entry of entries) {
                if (entry.contentRect) {
                    ...
                    requestAnimationFrame(() => updateSize(cachedWidthRef.current));
                }
            }
        });
        ...
        updateSize(cachedWidthRef.current);
