...
const HighResSpectrogram = memo(function HighResSpectrogram({ dataRef }) {
    ...
    // Optimization: Cache frequency bin map to avoid expensive calculations per frame
    ...
    const draw = useCallback(() => {
        ...
        // Optimization: Pre-calculate bin index map to avoid division per pixel
        if (!binIndexMapRef.current || binIndexMapRef.current.length !== height || binIndexMapRef.current.maxBin !== maxBin) {
            const map = new Int32Array(height);
            for (let i = 0; i < height; i++) {
                const freqRatio = (height - 1 - i) / height;
                map[i] = Math.floor(freqRatio * maxBin);
            }
            // Attach maxBin to the array instance to track changes
            map.maxBin = maxBin;
