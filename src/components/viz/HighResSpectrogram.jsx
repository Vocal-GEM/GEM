...
const HighResSpectrogram = memo(function HighResSpectrogram({ dataRef }) {
    ...
    const binIndexLookupRef = useRef({ height: 0, maxBin: 0, data: null });
    ...
    const draw = useCallback(() => {
        ...
        // Optimize: Precompute bin index lookup if needed
        if (!binIndexLookupRef.current.data ||
            binIndexLookupRef.current.height !== height ||
            binIndexLookupRef.current.maxBin !== maxBin) {
            ...
            binIndexLookupRef.current = { height, maxBin, data: lookup };
        }

        const binLookup = binIndexLookupRef.current.data;
