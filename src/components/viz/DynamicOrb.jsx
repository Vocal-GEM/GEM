...
const SafeModeVisualizer = memo(({ dataRef }) => {
  ...
  useEffect(() => {
    // ⚡ Bolt: Subscribing to renderCoordinator to prevent layout thrashing
    // This optimization centralizes the animation loop and reduces CPU overhead
    // by coordinating RAF calls instead of running an isolated loop.
    const loop = () => {
      if (dataRef.current) {
    ...
    const unsubscribe = renderCoordinator.subscribe(`SafeModeVisualizer-${componentId}`, loop);
    return () => unsubscribe();
  }, [dataRef, componentId]);
