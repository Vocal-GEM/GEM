...
// Memoized Visualizer Canvas to prevent re-renders when UI overlay updates
const VisualizerCanvas = memo(({ isVisible, mode, setMode, dataRef, externalDataRef, calibration, targetRange }) => {
  return (
    <Canvas
  ...
  return (
    <div className="w-full h-full flex items-center justify-center">
      <div className="relative w-64 h-64 flex items-center justify-center">
        ...
        {/* Core Circle */}
        <div ref={circleRef} className="relative w-32 h-32 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full shadow-[0_0_40px_rgba(59,130,246,0.5)] flex items-center justify-center transition-transform duration-100 will-change-transform">
          <div ref={textRef} className="text-white font-bold text-xl">
            ...
          </div>
        </div>
      </div>
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative group flex flex-col items-center justify-center">
      ...
      <div className="absolute top-4 left-4 z-10 flex gap-2 p-2 rounded-full bg-slate-900/50 backdrop-blur-md border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        {modes.map((m) => {
          ...
          return (
            <button
              ...
              className={`p-2 rounded-full transition-all focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-teal-400 ${isActive ? 'bg-teal-500 text-white shadow-[0_0_10px_rgba(20,184,166,0.5)]' : 'text-white/50 hover:text-white hover:bg-white/10'}`}
              title={m.label}
        ...
        {!beginnerMode && (
          <button
            ...
            className={`p-2 rounded-full transition-all focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-amber-400 ${showDebug ? 'bg-amber-500 text-white shadow-[0_0_10px_rgba(245,158,11,0.5)]' : 'text-white/50 hover:text-white hover:bg-white/10'}`}
      ...
      {(mode === 'safe' || calibration?.disable3D) && mode !== 'mixer' && mode !== 'gauges' ? (
        <SafeModeVisualizer dataRef={dataRef} />
