...
const SafeModeVisualizer = memo(({ dataRef }) => {
  ...
  useEffect(() => {
    // ⚡ Bolt: Replaced requestAnimationFrame with RenderCoordinator
    // 🎯 Why: Unifies animation loops to prevent layout thrashing and unnecessary renders
    // 📊 Impact: Improves frame rate stability and reduces CPU spikes in Safe Mode
    const loop = () => {
      if (dataRef.current) {
