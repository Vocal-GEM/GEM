      ...
      <div className="absolute top-4 left-4 z-10 flex gap-2 p-2 rounded-full bg-slate-900/50 backdrop-blur-md border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        {modes.map((m) => {
          ...
          return (
            <button
              ...
              className={`p-2 rounded-full transition-all ${isActive ? 'bg-teal-500 text-white shadow-[0_0_10px_rgba(20,184,166,0.5)]' : 'text-white/50 hover:text-white hover:bg-white/10'} focus:outline-none focus-visible:ring-2`}
              title={m.label}
              aria-label={m.label}
            >
              <Icon size={18} />
        ...
        {!beginnerMode && (
          <button
            ...
            className={`p-2 rounded-full transition-all ${showDebug ? 'bg-amber-500 text-white shadow-[0_0_10px_rgba(245,158,11,0.5)]' : 'text-white/50 hover:text-white hover:bg-white/10'} focus:outline-none focus-visible:ring-2`}
