...
const SafeModeVisualizer = memo(({ dataRef }) => {
  ...
  useEffect(() => {
    // Optimized: Subscribe to centralized RenderCoordinator instead of using independent requestAnimationFrame to prevent layout thrashing and lower CPU usage.
    const loop = () => {
      if (dataRef.current) {
    ...
    const unsubscribe = renderCoordinator.subscribe(componentId, loop);
    return () => unsubscribe();
  }, [dataRef, componentId]);
