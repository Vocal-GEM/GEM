...
const PitchOrb = ({ dataRef, settings = {} }) => {
    ...
    useEffect(() => {
        ...
        // ResizeObserver to track canvas dimensions asynchronously
        const resizeObserver = new ResizeObserver((entries) => {
            for (let entry of entries) {
                const { width, height } = entry.contentRect;
                if (width > 0 && height > 0) {
                    dimensionsRef.current = { width, height };
                    // We must re-assign canvas width/height outside the loop to prevent layout thrashing
                    // Note: doing so clears the canvas and resets the context state (like scale)
        ...
        const loop = () => {
            ...
            // Avoid getBoundingClientRect in high-frequency loop
