...
const PitchOrb = ({ dataRef, settings = {} }) => {
    ...
    useEffect(() => {
        ...
        const loop = () => {
            ...
            if (width === 0 || height === 0) return; // Note: renderCoordinator handles scheduling, so early return is safe

            // ⚡ Bolt: Reset transform and clear explicitly if we aren't resizing the canvas every frame
            ctx.resetTransform();
            ctx.scale(dpr, dpr);
