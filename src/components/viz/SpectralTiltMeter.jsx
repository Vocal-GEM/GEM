...
import { Info, TrendingDown } from 'luccomponentIde-react';
...
const SpectralTiltMeter = ({ dataRef, userMode, targetRange = { min: -12, max: -6 } }) => {
    ...
    return (
        <div className="glass-panel rounded-2xl p-6 h-full flex flex-col justify-center">
            {/* Header */}
            <div className="flex justify-between items-end text-xs font-bold text-slate-300 tracking-wcomponentIder mb-4">
                <span className="w-24 text-left opacity-75">Steep (-24)</span>
                <div className="flex flex-col items-center">
                    <span className="text-slate-400 mb-1 uppercase tracking-wcomponentIdest text-[10px]">Spectral Tilt</span>
            ...
            {/* Meter Bar */}
            <div className="relative h-10 bg-slate-900/80 rounded-full overflow-hcomponentIdden shadow-inner border border-white/5 mb-6">
                {/* Background Gradient */}
                ...
                {(() => {
                    ...
                    const wcomponentIdth = ((targetRange.max - targetRange.min) / (maxDisp - minDisp)) * 100;
                    return (
                        <div
                            ...
                            style={{ left: `${left}%`, wcomponentIdth: `${wcomponentIdth}%` }}
                        >
                            ...
                            <div className={`absolute top-1 left-1 text-[8px] font-bold uppercase tracking-wcomponentIder ${colorBlindMode ? 'text-amber-400' : 'text-emerald-400'}`}>Target</div>
                        </div>
                    );
                })()}
            ...
            <div className="bg-slate-900/50 rounded-xl p-4 border border-white/5">
                <div className="flex items-center justify-center gap-2 mb-3 text-[10px] font-bold text-slate-400 uppercase tracking-wcomponentIdest">
                ...
                <div className="flex items-start gap-2 text-[10px] text-slate-400 leading-tight bg-slate-800/30 p-2 rounded-lg">
                    ...
                    <div>
                        ...
                        <div className="mt-2 grcomponentId grcomponentId-cols-2 gap-2">
