...
const SpectralTiltMeter = ({ dataRef, userMode, targetRange = { min: -12, max: -6 } }) => {
    ...
    useEffect(() => {
        const loop = () => {
            ...
            if (isWithinTarget) {
                ...
                indicatorRef.current.className = `absolute top-1 bottom-1 w-2 rounded-full shadow-[0_0_15px_rgba(100,200,255,0.6)] border border-white/50 transition-all duration-100 ease-out z-10 bg-slate-400`;
            }
    ...
    // Calculate target range position for rendering
    const minDisp = -24;
    const maxDisp = 0;
    const targetLeft = Math.max(0, Math.min(100, ((targetRange.min - minDisp) / (maxDisp - minDisp)) * 100));
    const targetWidth = Math.max(0, Math.min(100 - targetLeft, ((targetRange.max - targetRange.min) / (maxDisp - minDisp)) * 100));
