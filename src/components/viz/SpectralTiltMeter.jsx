    ...
    useEffect(() => {
        const loop = () => {
            if (indicatorRef.current && valueRef.current) {
                ...
                if (isWithinTarget) {
                    indicatorRef.current.className = `absolute top-1 bottom-1 w-2 rounded-full border border-white/50 shadow-[0_0_15px_rgba(100,255,100,0.8)] transition-all duration-100 ease-out z-10 ${colorBlindMode ? 'bg-amber-500' : 'bg-emerald-500'}`;
                } else {
                    indicatorRef.current.className = `absolute top-1 bottom-1 w-2 rounded-full border border-white/50 shadow-[0_0_10px_rgba(100,200,255,0.8)] transition-all duration-100 ease-out z-10 bg-slate-400`;
                }
