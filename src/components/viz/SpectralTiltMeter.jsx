    ...
    useEffect(() => {
        const loop = () => {
            if (indicatorRef.current && valueRef.current) {
                ...
                if (isWithinTarget) {
                    indicatorRef.current.className = `absolute top-1 bottom-1 w-2 rounded-full border border-white/50 shadow-[0_0_15px_rgba(100,255,100,0.6)] transition-all duration-100 ease-out z-10 ${colorBlindMode ? 'bg-amber-500' : 'bg-emerald-500'}`;
                } else {
                    indicatorRef.current.className = `absolute top-1 bottom-1 w-2 rounded-full border border-white/50 shadow-[0_0_15px_rgba(100,200,255,0.6)] transition-all duration-100 ease-out z-10 bg-slate-400`;
                }
        ...
        <div className="glass-panel rounded-2xl p-6 h-full flex flex-col justify-center">
            ...
            <div className="relative h-10 bg-slate-900/80 rounded-full overflow-hidden shadow-inner border border-white/5 mb-6">
                ...
                {(() => {
                    const minDisp = -24;
                    const maxDisp = 0;
                    // Calculate left/width carefully
