    ...
    useEffect(() => {
        const loop = () => {
            if (indicatorRef.current && valueRef.current) {
                ...
                if (isWithinTarget) {
                    indicatorRef.current.className = `absolute top-1 bottom-1 w-2 rounded-full border border-white/50 transition-all duration-100 ease-out z-10 ${colorBlindMode ? 'bg-amber-400' : 'bg-emerald-400'}`;
                    indicatorRef.current.style.boxShadow = `0 0 15px ${colorBlindMode ? 'rgba(251,191,36,0.6)' : 'rgba(52,211,153,0.6)'}`;
                } else {
                    indicatorRef.current.className = `absolute top-1 bottom-1 w-2 rounded-full border border-white/50 transition-all duration-100 ease-out z-10 bg-slate-400`;
        ...
        <div className="glass-panel rounded-2xl p-6 h-full flex flex-col justify-center">
            ...
            <div className="relative h-10 bg-slate-900/80 rounded-full overflow-hidden shadow-inner border border-white/5 mb-6">
                ...
                {(() => {
                    ...
                    return (
                        <div
                            className={`absolute top-0 bottom-0 ${colorBlindMode ? 'bg-amber-500/20 border-x border-amber-500/30' : 'bg-emerald-500/20 border-x border-emerald-500/30'}`}
                        ...
                        >
                            ...
                            <div className="absolute top-1 left-1 text-[8px] font-bold uppercase tracking-wider text-white/50">Target</div>
                        </div>
                    );
                })()}
                ...
                <div
                    ref={indicatorRef}
                    className="absolute top-1 bottom-1 w-2 rounded-full border border-white/50 shadow-lg z-10"
                    style={{ left: '50%' }}
                />
            </div>
