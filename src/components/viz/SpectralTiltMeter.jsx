    ...
    useEffect(() => {
        const loop = () => {
            if (indicatorRef.current && valueRef.current) {
                ...
                // We can't read style.left here to avoid thrashing, but for this component
                // we'll just set it directly as it seems to be less critical or already optimized elsewhere?
                // Actually, let's just set it. The optimization in VoiceQualityMeter was specific to that component.
                ...
                if (isWithinTarget) {
                    indicatorRef.current.className = `absolute top-1 bottom-1 w-2 rounded-full shadow-[0_0_15px_rgba(100,255,100,0.6)] border border-white/50 transition-all duration-100 ease-out z-10 ${colorBlindMode ? 'bg-amber-500' : 'bg-emerald-500'}`;
                } else {
                    indicatorRef.current.className = "absolute top-1 bottom-1 w-2 rounded-full shadow-[0_0_15px_rgba(100,200,255,0.8)] transition-all duration-100 ease-out z-10 bg-slate-400";
                }
        ...
        <div className="glass-panel rounded-2xl p-6 h-full flex flex-col justify-center">
            ...
            <div className="relative h-10 bg-slate-900/80 rounded-full overflow-hidden shadow-inner border border-white/5 mb-6">
                ...
                {(() => {
                    const minDisp = -24;
                    const maxDisp = 0;
                    const rangeMin = Math.max(minDisp, targetRange.min);
                    const rangeMax = Math.min(maxDisp, targetRange.max);

                    const left = ((rangeMin - minDisp) / (maxDisp - minDisp)) * 100;
                    const width = ((rangeMax - rangeMin) / (maxDisp - minDisp)) * 100;
