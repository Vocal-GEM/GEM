...
const Spectrogram = ({ height = 200, showLabels = true }) => {
    ...
    const lutRef = useRef(null); // Lookup table for frequency bin mapping
    const lastMaxBinRef = useRef(0); // Track maxBin for LUT invalidation
    ...
    const draw = useCallback(() => {
        ...
        if (spectrum && spectrum.length > 0) {
            ...
            // --- OPTIMIZATION: Lookup Table (LUT) for Bin Mapping ---
            // Pre-calculate bin indices to avoid per-pixel float math in the loop.
            // Recalculate only if height or maxBin changes.
            if (!lutRef.current || lutRef.current.length !== h || lastMaxBinRef.current !== maxBin) {
