...
const StatCard = ({ label, value, subtext, icon, color }) => (
  <div className="bg-slate-900 p-4 sm:p-6 rounded-2xl border border-slate-800 hover:border-slate-700 transition-colors">
    <div className="flex justify-between items-start mb-2 sm:mb-4">
      ...
      {subtext && (
        <span className="text-[10px] sm:text-xs font-bold text-teal-500 bg-teal-500/10 px-2 py-1 rounded-full">
          {subtext}
        </span>
      )}
    </div>
...
const ActionCard = ({ title, description, onClick, icon, color }) => (
  ...
  >
    <div className="flex items-center justify-between mb-2 sm:mb-4">
      <div
        className={`p-2 sm:p-3 rounded-xl ${color} bg-opacity-10 group-hover:scale-110 transition-transform`}
      >
        {cloneElement(icon, { className: `${color} w-5 h-5 sm:w-6 sm:h-6` })}
      </div>
  ...
  return (
    <div className="w-full min-h-screen bg-black p-4 sm:p-6 lg:p-12 text-white">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/80 text-sm">
                  Quick routine to wake up your voice
                </p>
              </div>
            </div>
            <ArrowRight
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/80 text-sm">
                  Personalized session based on your weak areas
                </p>
              </div>
            </div>
            <ArrowRight
      ...
      <RecommendedExercises
        onViewCategory={(cat) => onViewChange("training", { category: cat })}
      />
