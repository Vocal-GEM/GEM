...
// Bolt Optimization: Extracted MetricCard from ClinicalAssessmentView and wrapped in React.memo
// to prevent unnecessary recreation and re-renders during high-frequency parent updates.
// Expected impact: Reduces React render cycle overhead and GC pressure.
const MetricCard = memo(({ label, value, unit, desc }) => (
...
`;
    ...
    const getGoalComparison = () => {
        ...
        if (pitch < goal.minPitch) {
            ...
            // For feminine/androgynous goals, being above is good (no ceiling)
            // Only masculine goal treats high pitch as a problem
            if (goalType === 'masculine') {
                return { status: 'above', diff: pitch - goal.maxPitch, label: 'Above Target' };
            }
            // For feminine goals, above max is still positive
            return { status: 'above_positive', diff: pitch - goal.maxPitch, label: 'Above Minimum ✓' };
        }
                            ...
                            {!results ? (
                                ...
                                <div className="space-y-6">
                                    ...
                                    {(() => {
                                        ...
                                        const goal = goalType === 'custom' ? customGoal : GOAL_PRESETS[goalType];
                                        const isPositive = comparison.status === 'within' || comparison.status === 'above_positive';
                                        return (
                                            <div className={`p-4 rounded-xl border ${isPositive
                                                ...
                                                <div className="flex items-center gap-2 mb-1">
                                                    <Flag size={16} className={isPositive ? 'text-green-400' : 'text-yellow-400'} />
                                                    <span className={`font-bold ${isPositive ? 'text-green-400' : 'text-yellow-400'}`}>
                                                        {comparison.label}
                                                    </span>
                                                </div>
                                                <div className="text-xs text-slate-400">
                                                    Goal: {goal.label} ({goal.minPitch} - {goal.maxPitch} Hz)
                                                    {comparison.diff > 0 && comparison.status !== 'above_positive' && (
                                                        <span className="ml-2 text-yellow-400">
                                                            ({comparison.diff} Hz {comparison.status === 'below' ? 'below target' : 'above target'})
                                                        </span>
                                                    )}
                                                    {comparison.status === 'above_positive' && (
                                                        <span className="ml-2 text-green-400">
                                                            (+{comparison.diff} Hz above minimum)
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
