            ...
            <div className={`flex flex-col w-full mx-auto ${embedded ? '' : 'bg-slate-900 rounded-2xl border border-white/10 max-w-lg shadow-2xl overflow-hidden'}`}>
                ...
                <div className="flex justify-between items-center p-4 border-b border-white/5 bg-slate-900/50">
                    <h2 className="text-lg sm:text-xl font-bold text-white flex items-center gap-2">
                        {phase !== 'select' && (
                            <button onClick={() => setPhase('select')} className="mr-2 text-slate-400 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded-full" aria-label="Back to phrase selection" title="Back to phrase selection">
                                <ArrowLeft size={20} />
                            </button>
                        )}
                        Shadowing & Mimicry
                    ...
                    {!embedded && onClose && (
                        <button onClick={onClose} className="p-2 text-slate-400 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded-full" aria-label="Close exercise" title="Close exercise">
                            <XCircle size={24} />
                        </button>
                    )}
                </div>
                    ...
                    {phase === 'record' && selectedClip && (
                        <div className="flex flex-col items-center justify-center space-y-8 py-8 animate-in fade-in zoom-in-95 duration-300">
                            ...
                            <button
                                ...
                                className={`w-24 h-24 rounded-full focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 flex items-center justify-center shadow-2xl transition-all transform hover:scale-105 active:scale-95 ${isRecording ? 'bg-red-500 border-4 border-red-400' : 'bg-blue-600 border-4 border-blue-500'
                                    }`}
                            >
                                {isRecording ? (
