            ...
            <div className={`flex flex-col w-full mx-auto ${embedded ? '' : 'bg-slate-900 rounded-2xl border border-white/10 max-w-lg shadow-2xl overflow-hidden'}`}>
                ...
                <div className="flex justify-between items-center p-4 border-b border-white/5 bg-slate-900/50">
                    <h2 className="text-lg sm:text-xl font-bold text-white flex items-center gap-2">
                        {phase !== 'select' && (
                            <button onClick={() => setPhase('select')} className="mr-2 text-slate-400 hover:text-white" aria-label="Go back to clip selection">
                                <ArrowLeft size={20} />
                            </button>
                        )}
                        Shadowing & Mimicry
                    ...
                    {!embedded && onClose && (
                        <button onClick={onClose} className="p-2 text-slate-400 hover:text-white" aria-label="Close exercise">
                            <XCircle size={24} />
                        </button>
                    )}
                </div>
