            ...
            <div className={`flex flex-col w-full mx-auto ${embedded ? '' : 'bg-slate-900 rounded-2xl border border-white/10 max-w-lg shadow-2xl overflow-hidden'}`}>
                ...
                <div className="flex justify-between items-center p-4 border-b border-white/5 bg-slate-900/50">
                    <h2 className="text-lg sm:text-xl font-bold text-white flex items-center gap-2">
                        {phase !== 'select' && (
                            <button aria-label="Go back to phrase selection" onClick={() => setPhase('select')} className="mr-2 text-slate-400 hover:text-white">
                                <ArrowLeft size={20} />
                            </button>
                        )}
                        Shadowing & Mimicry
                    ...
                    {!embedded && onClose && (
                        <button aria-label="Close Shadowing Exercise" onClick={onClose} className="p-2 text-slate-400 hover:text-white">
                            <XCircle size={24} />
                        </button>
                    )}
                </div>
                        ...
                        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="grid gap-3">
                                {TARGET_CLIPS.map(clip => (
                                    <button
                                        key={clip.id}
                                        aria-label={`Select phrase: ${clip.text}`}
