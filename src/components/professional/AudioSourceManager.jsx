...
const AudioSourceManager = ({ onSourceChange }) => {
    ...
    const enumerateDevices = useCallback(async () => {
        try {
            ...
            // We need to be careful not to trigger infinite loops if onSourceChange changes identity
            if (audioInputs.length > 0) {
                // Check if currently selected device still exists
                const currentExists = audioInputs.some(d => d.deviceId === selectedDeviceId);
                if (!currentExists || !selectedDeviceId) {
                     const firstDevice = audioInputs[0].deviceId;
                     setSelectedDeviceId(firstDevice);
                     if (onSourceChange) onSourceChange(firstDevice);
                }
            }
        } catch (err) {
