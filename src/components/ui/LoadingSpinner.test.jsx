...
describe("LoadingSpinner", () => {
  ...
  it("renders with different sizes", () => {
    ...
    expect(container.firstChild).toHaveClass('inline-flex'); // sm is inline

    rerender(<LoadingSpinner size="xl" />);
    expect(screen.getByRole("status")).toBeInTheDocument();
    expect(container.firstChild).toHaveClass('flex'); // xl is flex

    rerender(<LoadingSpinner size="xs" />);
    expect(screen.getByRole("status")).toBeInTheDocument();
    expect(container.firstChild).toHaveClass('inline-flex'); // xs is inline
  });
