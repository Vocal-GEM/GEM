...
describe("LoadingSpinner", () => {
  ...
  it("renders with different sizes", () => {
    ...
    // Check sizing classes implicitly or via snapshot if needed,
    // but here just checking it renders without crashing is fine for "different sizes" test logic
    // unless we check specific classes.
    expect(container.firstChild).toHaveClass("inline-flex"); // sm/xs uses inline-flex

    rerender(<LoadingSpinner size="xl" />);
    expect(screen.getByRole("status")).toBeInTheDocument();
    expect(container.firstChild).toHaveClass("flex"); // larger uses flex
  });
