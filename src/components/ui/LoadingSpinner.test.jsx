...
describe("LoadingSpinner", () => {
  ...
  it("renders with different sizes", () => {
    ...
    const spinnerSm = screen.getByRole("status");
    expect(spinnerSm.firstChild).toHaveClass("w-6"); // sm is w-6

    rerender(<LoadingSpinner size="xl" />);
    const spinnerXl = screen.getByRole("status");
    expect(spinnerXl.firstChild).toHaveClass("w-24"); // xl is w-24

    rerender(<LoadingSpinner size="xs" />);
    const spinnerXs = screen.getByRole("status");
    expect(spinnerXs.firstChild).toHaveClass("w-4"); // xs is w-4
  });
