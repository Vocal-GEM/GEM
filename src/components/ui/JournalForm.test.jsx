import { render, screen, getAllByLabelText } from '@testing-library/react';
...
describe('JournalForm Accessibility', () => {
  ...
  it('has accessible label for Effort slider', () => {
    render(<JournalForm />);
    // Use getAllByLabelText in case multiple elements match, and take the first one or assert on length
    // If we expect one, getByLabelText is fine, but if duplicates exist, we fix test or code.
    // Assuming cleaner code now.
    expect(screen.getAllByLabelText(/effort/i)[0]).toBeInTheDocument();
  });

  it('has accessible label for Confidence slider', () => {
    render(<JournalForm />);
    expect(screen.getAllByLabelText(/confidence/i)[0]).toBeInTheDocument();
  });
