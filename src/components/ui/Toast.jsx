...
const Toast = ({ message, type = 'success', onClose, duration = 3000, className }) => {
  ...
  return (
    ...
    >
      <Icon className={clsx('w-5 h-5 shrink-0', style.text)} aria-hidden="true" />
      <span className="sr-only">{style.label}: </span>
      <span className={clsx('font-medium', style.text)}>{message}</span>
      <button
        onClick={onClose}
        className={clsx(
          'ml-2 hover:opacity-70 p-1 rounded-full transition-opacity focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 focus-visible:ring-current',
          style.text
        )}
