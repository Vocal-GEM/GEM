...
const Button = React.forwardRef(
  (
    ...
    const Comp = asChild ? "span" : "button"; // Simple placeholder for Slot if not using Radix UI Slot
    ...
    return (
      ...
      >
        {isLoading ? (
          isIcon ? (
            <LoadingSpinner size={spinnerSize} variant="current" label="Loading" />
          ) : (
            <>
              <LoadingSpinner size={spinnerSize} variant="current" className="mr-2" label="Loading" />
              {children}
            </>
          )
        ) : (
