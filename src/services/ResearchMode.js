...
export class ResearchModeController {
    ...
    generateParticipantId(userId) {
        ...
        const researchSalt = typeof process !== 'undefined' && process.env.REACT_APP_RESEARCH_SALT
            // eslint-disable-next-line no-undef
            ? process.env.REACT_APP_RESEARCH_SALT
            : import.meta.env.VITE_RESEARCH_SALT;

        if (!researchSalt) {
            throw new Error('Research salt is required for participant ID generation');
        }
