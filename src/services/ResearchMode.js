...
export class ResearchModeController {
    ...
    generateParticipantId(userId) {
        ...
        const salt = this.studyId + (typeof process !== 'undefined' && process.env ? process.env.REACT_APP_RESEARCH_SALT : import.meta.env.VITE_APP_RESEARCH_SALT);
        if (!salt) {
            throw new Error('Research salt is missing');
        }
