...
export class ResearchModeController {
    ...
    generateParticipantId(userId) {
        ...
        const envSalt = import.meta.env.VITE_RESEARCH_SALT || process.env.REACT_APP_RESEARCH_SALT;

        if (!envSalt) {
            throw new Error('VITE_RESEARCH_SALT environment variable is required for secure anonymization');
        }
