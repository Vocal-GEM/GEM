...
export class ResearchModeController {
    ...
    generateParticipantId(userId) {
        ...
        const envSalt = import.meta.env?.VITE_RESEARCH_SALT || (typeof process !== 'undefined' && process.env?.REACT_APP_RESEARCH_SALT);
        if (!envSalt) {
            throw new Error('Research mode requires a cryptographic salt in environment variables (VITE_RESEARCH_SALT).');
        }
