    ...
    process(inputs, outputs, parameters) {
        ...
        for (let i = 0; i < inputChannel.length; i++) {
            ...
            if (this.bufferIndex >= this.bufferSize) {
                ...
                const currentWorkletTime = typeof currentTime !== 'undefined' ? currentTime : Date.now() / 1000;
                const startTime = currentWorkletTime;

                // Detect pitch using YIN algorithm
                const result = this.detectPitchYIN(this.buffer);

                const processingTime = (currentWorkletTime - startTime) * 1000; // Convert to ms
