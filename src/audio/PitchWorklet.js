    ...
    process(inputs, _outputs, _parameters) {
        ...
        for (let i = 0; i < inputChannel.length; i++) {
            ...
            if (this.bufferIndex >= this.bufferSize) {
                const startTime = currentTime / this.sampleRate;

                // Detect pitch using YIN algorithm
                const result = this.detectPitchYIN(this.buffer);

                const processingTime = (currentTime / this.sampleRate - startTime) * 1000; // Convert to ms
                ...
                this.port.postMessage({
                    ...
                    timestamp: currentTime / this.sampleRate,
