    ...
    process(inputs, outputs, parameters) {
        ...
        for (let i = 0; i < inputChannel.length; i++) {
            ...
            if (this.bufferIndex >= this.bufferSize) {
                ...
                const currentAudioTime = typeof currentTime !== 'undefined' ? currentTime : performance.now() / 1000;
                const startTime = currentAudioTime;

                // Detect pitch using YIN algorithm
                const result = this.detectPitchYIN(this.buffer);

                const processingTime = (currentAudioTime - startTime) * 1000; // Convert to ms
