    ...
    process(inputs, outputs, parameters) {
        ...
        for (let i = 0; i < inputChannel.length; i++) {
            ...
            if (this.bufferIndex >= this.bufferSize) {
                ...
                const safeCurrentTimeStart = typeof currentTime !== 'undefined' ? currentTime : Date.now() / 1000;
                const startTime = safeCurrentTimeStart;

                // Detect pitch using YIN algorithm
                const result = this.detectPitchYIN(this.buffer);

                // eslint-disable-next-line no-undef
                const safeCurrentTimeEnd = typeof currentTime !== 'undefined' ? currentTime : Date.now() / 1000;
                const processingTime = (safeCurrentTimeEnd - startTime) * 1000; // Convert to ms
