    ...
    process(inputs, _outputs, _parameters) {
        ...
        for (let i = 0; i < inputChannel.length; i++) {
            ...
            if (this.bufferIndex >= this.bufferSize) {
                // Use globalThis.currentTime if available, otherwise fallback or mock
                const time = typeof currentTime !== 'undefined' ? currentTime : globalThis.currentTime;
                const startTime = time;

                // Detect pitch using YIN algorithm
                const result = this.detectPitchYIN(this.buffer);

                // Use current time again for end measurement
                const endTime = typeof currentTime !== 'undefined' ? currentTime : globalThis.currentTime;
