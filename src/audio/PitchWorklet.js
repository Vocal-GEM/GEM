    ...
    process(inputs, _outputs, _parameters) {
        ...
        for (let i = 0; i < inputChannel.length; i++) {
            ...
            if (this.bufferIndex >= this.bufferSize) {
                // Use globalThis.currentTime or implicit global currentTime
                // ESLint doesn't know about AudioWorkletGlobalScope without config
                const now = globalThis.currentTime || currentTime;

                // Detect pitch using YIN algorithm
                const result = this.detectPitchYIN(this.buffer);

                const processingTime = ((globalThis.currentTime || currentTime) - now) * 1000; // Convert to ms
