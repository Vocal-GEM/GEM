...
export class LPCAnalyzer {
    constructor(order = 12, sampleRate = 48000) {
        ...
        // Pre-allocate buffers to prevent GC churn in real-time processing
        this.preEmphasized = new Float32Array(0);
        this.windowed = new Float32Array(0);
        ...
        this.magnitudeBuffer = new Float32Array(512);
    }
    ...
    applyPreEmphasis(signal, coeff = 0.97) {
        if (this.preEmphasized.length !== signal.length) {
            this.preEmphasized = new Float32Array(signal.length);
        }
        const output = this.preEmphasized;
        output[0] = signal[0];
    ...
    applyWindow(signal) {
        ...
        if (this.windowed.length !== N) {
            this.windowed = new Float32Array(N);
        }
        const output = this.windowed;
        for (let i = 0; i < N; i++) {
            // Hamming window
    ...
    computeAutocorrelation(signal, order) {
        if (this.rBuffer.length < order + 1) {
            this.rBuffer = new Float32Array(order + 1);
        }
    ...
    levinsonDurbin(R, order) {
        if (this.aBuffer.length < order + 1) {
            this.aBuffer = new Float32Array(order + 1);
            this.eBuffer = new Float32Array(order + 1);
            this.kCoeffBuffer = new Float32Array(order + 1);
            this.aPrevBuffer = new Float32Array(order + 1);
        }
        ...
        return { a: a.slice(1, order + 1), error: E[order] }; // Return safely sliced array (new reference)
    }
    ...
    computeLPCSpectrum(a, error, numPoints) {
        ...
        if (gain < 1e-10) {
            magnitude.fill(-100);
            return new Float32Array(magnitude); // Return a new copy for referential equality and state safety
        }
        ...
        return new Float32Array(magnitude); // Return a new copy to ensure React renders correctly
    }
