...
export class VoiceAnalyzer {
    ...
    estimateHNR(samples, sampleRate) {
        ...
        // Only compute autocorrelation up to maxLag instead of the full sample length
        // This changes an O(N^2) operation to O(N * maxLag), reducing computation by >50%
        const autocorr = this.autocorrelate(samples, maxLag);
     ...
     * @param {number} maxLag - Maximum lag to compute (optional, defaults to full length)
     */
    autocorrelate(samples, maxLag = samples.length) {
