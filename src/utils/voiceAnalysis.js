...
export class VoiceAnalyzer {
    ...
    estimateHNR(samples, sampleRate) {
        // Optimize: only calculate autocorrelation up to max needed lag
        const minLag = Math.floor(sampleRate / 600);
        const maxLag = Math.floor(sampleRate / 75);

        // Pass maxLag + 1 because we need to read up to maxLag index inclusive
     ...
     * @param {number} maxLag - Maximum lag to compute (performance optimization)
     */
    autocorrelate(samples, maxLag = samples.length) {
        const limit = Math.min(maxLag, samples.length);
