...
export class VoiceAnalyzer {
    ...
    estimateHNR(samples, sampleRate) {
        ...
        // ⚡ Bolt Optimization: Only autocorrelate up to the maximum lag we care about
        // This changes O(N^2) to O(N * maxLag), a massive speedup for large buffers
     ...
     * @param {number} maxLagLimit Optional max lag to optimize computation
     */
    autocorrelate(samples, maxLagLimit) {
        const limit = maxLagLimit ? Math.min(maxLagLimit, samples.length) : samples.length;
