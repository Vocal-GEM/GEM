...
def validate_u_vowel(y, sr, f1_target=300, f2_target=800, tolerance=0.25):
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error(f"Error checking audio: {e}")
        return {"is_valid": False, "error": "An internal error occurred."}
...
def analyze_file(path, goal_name="transfem_soft_slightly_breathy"):
    ...
    if _voicelab_available:
        try:
            ...
            import logging
            logging.getLogger(__name__).error(f"Error in voicelab analysis: {e}")
            voicelab_data = {"error": "An internal error occurred."}
