...
def share_voice():
    ...
    try:
        ...
        # Save the file first so anonymizer can read it
        ...
        try:
            ...
            current_app.logger.error(f"Anonymization failed: {e}")
            # If anonymization fails, we must abort and not share the original
            return jsonify({'error': 'Failed to process audio for privacy'}), 500
        finally:
