...
def share_voice():
    ...
    try:
        ...
        try:
            ...
            current_app.logger.error(f"Failed to anonymize: {e}")
            # Ensure cleanup happens even if anonymization fails
...
def get_shared_voice(share_id):
    ...
    try:
        ...
        # Check expiration
        if sample.expires_at and datetime.utcnow() > sample.expires_at:
