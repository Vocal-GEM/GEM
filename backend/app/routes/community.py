...
def share_voice():
    ...
    try:
        ...
        anon_filepath = None
        try:
            # First, save the file so librosa can load it
            audio_file.save(filepath)

            # Reset file pointer just in case, though save consumes it usually.
            # But librosa loads from path, so it's fine.
            ...
            # If anonymization failed, we must not proceed with sharing
            ...
            # This runs whether success or exception occurred
        ...
        # If we got here but anon_filepath is None (e.g. exception caught above but returned),
        # actually the return 500 above handles it.
        # But let's be safe.
        if not anon_filepath:
             return jsonify({'error': 'Failed to anonymize voice sample'}), 500
