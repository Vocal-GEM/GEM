...
def clean_audio():
    ...
    try:
        ...
        # Schedule cleanup after response using standard utility
        ...
        # Manual cleanup on error since cleanup_file_after_request might not run if we crash before return
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
            except Exception as cleanup_error:
                current_app.logger.error(f"Error cleaning up temp file: {cleanup_error}")
...
def manipulate_file():
    ...
    try:
        ...
        if manipulated is None:
             # Cleanup specific to this failure path
             if tmp_path and os.path.exists(tmp_path):
                 try:
                    os.remove(tmp_path)
                 except: pass
        ...
        # Cleanup original input file immediately as we are done with it
        if tmp_path and os.path.exists(tmp_path):
             try:
                os.remove(tmp_path)
             except: pass
        ...
        if tmp_path and os.path.exists(tmp_path):
            try:
                ...
                current_app.logger.error(f"Error cleaning up tmp file: {cleanup_error}")
