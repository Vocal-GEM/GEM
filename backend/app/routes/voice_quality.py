        ...
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
                 ...
                 current_app.logger.error(f"Error removing temp file during exception handling: {cleanup_error}")
...
def manipulate_file():
    ...
    try:
        ...
        if manipulated is None:
             # Cleanup temp if we are returning early
             if tmp_path and os.path.exists(tmp_path):
                 os.remove(tmp_path)
        ...
        if processed_path and os.path.exists(processed_path):
             try:
                ...
                current_app.logger.error(f"Error removing processed file: {cleanup_error}")

        if tmp_path and os.path.exists(tmp_path):
             try:
                os.remove(tmp_path)
             except Exception as cleanup_error:
                current_app.logger.error(f"Error removing temp file: {cleanup_error}")
