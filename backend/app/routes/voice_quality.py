...
def clean_audio():
    ...
    try:
        ...
        # Schedule cleanup after response via utils (backup)
        ...
        if tmp_path and os.path.exists(tmp_path):
            try:
                ...
                current_app.logger.error(f"Error cleaning up temp file on failure: {cleanup_error}")
...
def manipulate_file():
    ...
    try:
        ...
        if manipulated is None:
             # This is an internal logic error, so log and return 500
             current_app.logger.error("Manipulation failed (returned None)")
        ...
        # Remove the original temp file now, as we don't need it anymore
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
                tmp_path = None # Prevent double cleanup
            except Exception as e:
                current_app.logger.error(f"Error removing temp file: {e}")
        ...
        if processed_path and os.path.exists(processed_path):
             try:
                ...
                current_app.logger.error(f"Error removing processed file on failure: {cleanup_error}")

        if tmp_path and os.path.exists(tmp_path):
             try:
                os.remove(tmp_path)
             except Exception as cleanup_error:
                current_app.logger.error(f"Error removing temp file on failure: {cleanup_error}")
