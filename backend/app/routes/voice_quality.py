...
def clean_audio():
    ...
    try:
        ...
        # Also schedule with cleanup utility as backup
        ...
        # Clean up on error since after_request might not run if we return 500
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
            except Exception:
        ...
        # Security: Do not expose internal error details to client (CWE-209 Fix)
...
def manipulate_file():
    ...
    try:
        ...
        if manipulated is None:
             # Clean up tmp_path if manipulation failed
             if tmp_path and os.path.exists(tmp_path):
                 os.remove(tmp_path)
        ...
        # Cleanup original temp file immediately as we don't need it for send_file
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
            except Exception:
                pass
            tmp_path = None # Mark as removed

        # Schedule cleanup for processed file
        ...
        # Cleanup original temp file if still exists
        if tmp_path and os.path.exists(tmp_path):
             try:
                os.remove(tmp_path)
             except Exception:
        ...
        # CWE-209 Fix: Generic error message
