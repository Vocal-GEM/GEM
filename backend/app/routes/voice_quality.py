...
def clean_audio():
    ...
    try:
        ...
        # Schedule cleanup via external util if needed (redundancy)
        ...
        # Security: Return generic error message instead of str(e)
...
def manipulate_file():
    ...
    try:
        ...
        if manipulated is None:
             # Remove tmp_path as we are returning early
             if os.path.exists(tmp_path):
                 os.remove(tmp_path)
        ...
        # Ensure original temp file is always cleaned up (if not cleaned up in except block)
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
            except OSError:
