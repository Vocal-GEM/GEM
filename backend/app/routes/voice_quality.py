...
def manipulate_file():
    ...
    try:
        ...
        # Cleanup original temp file immediately if it exists (extra safety)
        # Note: cleanup_file_after_request handles deferred cleanup, but we want to ensure immediate cleanup if sending fails or exception occurs before cleanup registration
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
            except Exception:
