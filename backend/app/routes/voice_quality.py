    ...
    try:
        ...
        if os.path.exists(tmp_path):
            try:
                ...
                current_app.logger.warning(f"Failed to remove temp file {tmp_path}: {e}")
...
def clean_audio():
    ...
    try:
        ...
        # Schedule cleanup after response (backup)
        ...
        if tmp_path and os.path.exists(tmp_path):
            try:
                ...
                current_app.logger.warning(f"Failed to remove temp file on error: {cleanup_error}")
    ...
    try:
        ...
        if processed_path and os.path.exists(processed_path):
             try:
                ...
                current_app.logger.warning(f"Failed to remove processed file on error: {cleanup_error}")
        ...
        if tmp_path and os.path.exists(tmp_path):
            try:
                ...
                current_app.logger.warning(f"Failed to remove temp file in finally: {e}")
