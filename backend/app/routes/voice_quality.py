    ...
    try:
        ...
        if tmp_path and os.path.exists(tmp_path):
            try:
                ...
                current_app.logger.warning(f"Failed to cleanup temp file {tmp_path}: {e}")
        ...
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
                 ...
                 current_app.logger.warning(f"Failed to cleanup temp file {tmp_path}: {cleanup_error}")
...
def manipulate_file():
    ...
    try:
        ...
        if manipulated is None:
             # Clean up temp path before returning
             if tmp_path and os.path.exists(tmp_path):
                 os.remove(tmp_path)
        ...
        if processed_path and os.path.exists(processed_path):
             try:
                ...
                current_app.logger.warning(f"Failed to cleanup processed file {processed_path}: {cleanup_error}")
