...
def synthesize_speech():
    ...
    try:
        ...
        current_app.logger.error(f"ElevenLabs synthesize error: {str(e)}")
        return jsonify({"error": "Failed to connect to ElevenLabs API."}), 502
...
def get_voices():
    ...
    try:
        ...
        # Security: Log raw exception internally to prevent info leakage
        current_app.logger.error(f"ElevenLabs get_voices error: {str(e)}")
        return jsonify({"error": "Failed to connect to ElevenLabs API.", "voices": []}), 502
