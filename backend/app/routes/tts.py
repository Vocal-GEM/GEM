...
def synthesize_speech():
    ...
    try:
        ...
        current_app.logger.error(f"ElevenLabs connection failed: {str(e)}")
        return jsonify({"error": "Failed to connect to speech service. Please try again later."}), 502
...
def get_voices():
    ...
    try:
        ...
        from flask import current_app
        current_app.logger.error(f"ElevenLabs connection failed: {str(e)}")
        return jsonify({"error": "Failed to connect to speech service.", "voices": []}), 502
