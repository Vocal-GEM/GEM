...
def synthesize_speech():
    ...
    try:
        ...
        current_app.logger.error(f"ElevenLabs TTS synthesis error: {str(e)}")
        return jsonify({"error": "Failed to connect to ElevenLabs service"}), 502
...
def get_voices():
    ...
    try:
        ...
        current_app.logger.error(f"ElevenLabs fetch voices error: {str(e)}")
        return jsonify({"error": "Failed to fetch voices from ElevenLabs service", "voices": []}), 502
