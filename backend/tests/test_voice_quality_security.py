...
class TestVoiceQualitySecurity(unittest.TestCase):
    def setUp(self):
        ...
        # Configure voice analysis mocks
        sys.modules['backend.app.voice_quality_analysis'].GOAL_PRESETS = {}
        sys.modules['backend.app.voice_quality_analysis'].load_audio.return_value = (MagicMock(), 44100)
        sys.modules['backend.app.voice_quality_analysis'].clean_audio_signal.return_value = MagicMock()
    ...
    def test_manipulate_file_success(self):
        ...
        with patch('backend.app.services.voicelab_service.manipulate_voice', return_value=mock_sound):
            with patch('parselmouth.Sound', return_value=MagicMock()):
                # Mock send_file to avoid actual file system ops failing
    ...
    def test_manipulate_file_error_handling_generic(self):
        """
        ...
        # We need to ensure the mocked module raises this
    ...
    def test_clean_audio_error_handling_generic(self):
        """
        Test that an error in cleaning is handled safely.
        """
        file_content = b'fake audio data'
        file_storage = FileStorage(
            stream=BytesIO(file_content),
        ...
        secret_message = "SECRET_CLEAN_FAIL"
        sys.modules['backend.app.voice_quality_analysis'].clean_audio_signal.side_effect = Exception(secret_message)
